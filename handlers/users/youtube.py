...
class YouTube:
    ...
    def ForConvert(self):
        try:
            response = requests.post("https://x2download.app/api/ajaxSearch", headers=self.headers,
                                     data={"q": self.url, 'vt': 'home'})
    ...
    def Convert(self):
        ...
        payload = {'v_id': media[0], 'ftype': 'mp4', 'fquality': '480p', 'fname': media[1], 'token': media[2],
                   'timeExpire': media[3]}
