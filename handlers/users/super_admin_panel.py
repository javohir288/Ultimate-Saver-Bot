...
async def add_admin_method(message: types.Message, state: FSMContext):
    try:
        ...
        await bot.send_message(chat_id=adminid,
                               text="tabriklaymiz siz botimizda adminlik huquqini qolgan kiritidingiz /start bosin")
    ...
    if matn.isdigit() or matn.startswith("@"):
        try:
            if db.check_channel(channel=message.text):
                await message.answer("<i>❌Bu kanal qo'shilgan! Boshqa kanal qo'shing!</i>",
                                     reply_markup=back_to_main_menu)
            else:
                try:
...
async def stat(call: types.CallbackQuery):
    ...
    for x in stat:
        ...
        await call.message.answer(f"<b>👥 Bot foydalanuvchilari soni: {(x)} nafar\n</b>"
                                  ...
                                  f"<b>📆 Sana: {yil_oy_kun}</b>", reply_markup=types.InlineKeyboardMarkup().add(
